...
class QuizUserResponseType extends AbstractType
{
    ...
    {
                // Récupérer les questions passées en option
                $questions = $options['questions'];

                // Ajouter les champs pour chaque question
                foreach ($questions as $question) {
                    $reponses = $question->getReponses(); // Supposons que getReponses() retourne les réponses liées
        
                    $choices = [];
                    foreach ($reponses as $reponse) {
                        $choices[$reponse->getAnswerText()] = $reponse->getId(); // Utiliser l'ID de la réponse comme valeur
                    }
        // $questions = $options['questions'];
